	...
	methods:{
		checkCorpname(){
			...
			if(_this.corpname===''||_this.corpname===null){
				...
				axiosApi.axiosPost('/apis/platform/corp/register/check',
					{
		...
		submit(direction){
			...
			if(_this.checkCorpnameState=="ok"&&_this.checkPasswordState=="ok"&&_this.checkComfirmPasswordState=="ok"&&_this.checkEmailState=="ok"){
				axiosApi.axiosPost('/apis/platform/corp/register/create',
					{
						'corpname':_this.corpname,
						"email": _this.email,
						...
						if(res.data.errmsg=="ok"){
							...
							if(direction==1){
								 _this.$router.push({path:'/www/UnitReg2',query: {id: res.data.data._id}})
							}else{
								_this.$router.push({path:'/www'})
							}
						}else{
